...
function lancerJeu() {
    ...
    // Gérer l'événement input sur le champ de saisie
    inputEcriture.addEventListener("input", () => {
        // Comparer le contenu du champ avec le mot actuel
        if (inputEcriture.value.toLowerCase() === listeProposition[i].toLowerCase()) {
            // Si le mot est correct, incrémenter l'indice et le score et passer au mot suivant
            i++;
            score++;
            afficherResultat(score, i);
            inputEcriture.value = ''; // Effacer le contenu du champ de saisie
            // Vérifier si tous les mots ont été proposés
            if (listeProposition[i] === undefined) {
                afficherProposition("Le jeu est fini");
